...
function KnightSkillAddLife(Vitality, Energy, PartyBonus)
	local SkillEffect = Vitality / 100 + 12 + Energy / 100 + PartyBonus
...
function SummonerBerserkerAttackDamage(Strength, Dexterity)
	local AttackMin = (Strength+Dexterity) / 14
	local AttackMax = (Strength+Dexterity) / 8
...
function SummonerBerserkerMagicDamage(Effect, Energy)
	local MagicMin = (Energy / 18) * Effect / 100
	local MagicMax = (Energy / 8) * Effect / 100
...
function SummonerBerserkerCurseDamage(Effect, Energy)
	local CurseMin = Effect * (Energy / 18) / 100
	local CurseMax = Effect * ((Energy + 0.015) / 8) / 100
...
function Sleep_Monster(Energy, Curse, MonsterLevel)
	...
	local SkillTime = Energy / 300 + 5 - MonsterLevel / 20
...
function Sleep_PvP(Energy, Curse, PlayerLevel, TargetLevel)
	...
	local SkillTime = Energy / 1250 + (PlayerLevel - TargetLevel) / 100 + 4
